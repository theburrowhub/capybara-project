...
# Quick test script for audio analyzer (console version)
# This will run the analyzer for the first 10 seconds of a selected file

echo "Testing audio bass analyzer (console version)..."
echo ""

# Check if binary exists
if [ ! -f "bin/audio_analyzer" ]; then
...
if [ -f "assets/audio/level1.mp3" ]; then
    ...
    echo "=================================================="
    echo ""
    ./bin/audio_analyzer -f level1.mp3
else
...
echo "  ./bin/audio_bass_analyzer -f <filename.mp3>"
echo "  ./bin/audio_bass_analyzer -f level1.mp3 -t 30  # Analyze first 30 seconds"
echo ""
echo "To run the full GUI demo, use:"
